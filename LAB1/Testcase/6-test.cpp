...
public:
    // Constructor to initialize arrays A and B
    SubsequenceCounter(const std::vector<int>& a, const std::vector<int>& b) : A(a), B(b) {}

    // Function to find and return the count and starting indices of subsequence occurrences
...
void runTestCase(const std::vector<int>& A, const std::vector<int>& B, int expectedCount, const std::vector<int>& expectedIndices) {
    ...
    for (int idx : expectedIndices) std::cout << idx << " ";
    std::cout << "}, Got: " << result.first << " times at indices {";
    for (int idx : result.second) std::cout << idx << " ";
    std::cout << "}\n";
}
